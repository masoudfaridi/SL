...
An important state space model is the unobserved components model. In the first
level, the (real-valued) observable $y_t$ at time $t$ is modeled to depend on the state or
unobserved component $\pi_t$ as follows:
...
An important unobserved components model
is random walk as follows:
...
\end{eqnarray}
 ...
 where $u_t \stackrel{i.i.d}{\sim} N(0,w^2)$. That is, the conditional distribution of
$\tau_t$ given $\tau_{t-1}$ and $w^2$ is $N(u_{t-1},w^2)$ the current state $\tau_t$ centers around the previous
one $\tau_{t-1}$ while $w^2$ controls how close the two terms are on average.